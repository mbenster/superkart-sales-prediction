...
# "backend" is the *container name* assigned when the backend container is started on
# the shared Docker network; Docker's internal DNS resolves it to that container's IP.
...
# Input fields for product and store data - one widget per model input feature
...
# Create JSON payload matching the 10 features the model expects
product_data = {
